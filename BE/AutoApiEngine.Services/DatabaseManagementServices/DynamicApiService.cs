...
namespace AutoApiEngine.Services.DatabaseManagementServices
{
    ...
    public class DynamicApiService : IDynamicApiService
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (data.Count == 0)
                throw new ArgumentException("Request body must contain at least one column-value pair.");

            ValidateColumns(meta, data.Keys);

            var qi = QuoteIdentifier(workspace.DatabaseEngine);
            var qiClose = qi == "[" ? "]" : qi;
            var columns = string.Join(", ", data.Keys.Select(c => $"{qi}{c}{qiClose}"));
            var paramNames = string.Join(", ", data.Keys.Select((c, i) => $"@p{i}"));
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var setClauses = data.Keys.Select((c, i) => $"{qi}{c}{qiClose} = @p{i}");
            var parameters = new List<DbParameter>();
            var idx = 0;
            foreach (var kvp in data)
            {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var sql = $"DELETE FROM {qi}{meta.Schema}{qiClose}.{qi}{objectName}{qiClose} OUTPUT DELETED.* WHERE {qi}{pkCol}{qiClose} = @pk";
        ...
        internal record ColumnInfo(string Name, string DataType, bool IsNullable, bool IsPrimaryKey);
        ...
        internal async Task<TableMetadata> GetTableMetadataAsync(Workspace workspace, string objectName, CancellationToken ct)
        {
            ...
            await using (var cmd = connection.CreateCommand())
            {
                ...
                {
                    var col = new ColumnInfo(reader.GetString(0), reader.GetString(1), reader.GetString(2) == "YES", reader.GetInt32(3) == 1);
            ...
            DbConnection connection, DatabaseEngine engine, string schema, string objectName, CancellationToken ct)
        {
            ...
            await using var reader = await cmd.ExecuteReaderAsync(ct);
            while (await reader.ReadAsync(ct))
                columns[reader.GetString(0)] = new ColumnInfo(reader.GetString(0), reader.GetString(1), reader.GetString(2) == "YES", false);
